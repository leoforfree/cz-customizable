...
function findConfigFile(CZ_CONFIG_NAME) {
  const extensions = ['.js', '.cjs'];
  let result = null;
  extensions.some((ext) => {
    const filename = findConfig.require(CZ_CONFIG_NAME + ext, { home: false });
    if (filename !== null) {
      result = filename;
      return true;
    }
...
const readConfigFile = (CZ_CONFIG_NAME = '.cz-config') => {
  ...
  const czConfig = findConfigFile(CZ_CONFIG_NAME);
